...
/*   Updated: 2020/02/29 19:47:52 by tembu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			check_flag_nb_minus_star(const char *str,
				t_flag *my_struct, va_list args, int *i)
{
	while ((str[*i] >= '0' && str[*i] <= '9')
	|| str[*i] == '-' || str[*i] == '*')
	{
		if (str[*i] >= '0' && str[*i] <= '9' && (++my_struct->len))
		{
			while (str[*i] >= '0' && str[*i] <= '9')
			{
		...
		if (str[*i] == '*')
		{
			...
			if (my_struct->star < 0 && (my_struct->minus = 1))
				my_struct->star *= (-1);
			my_struct->len += 1;
			*i += 1;
		}
				...
				t_flag *my_struct, va_list args, int *i)
{
	...
	|| str[*i] == '-' || str[*i] == '*')
	{
		...
		if (str[*i] == '*')
		{
			...
			if (my_struct->star2 < 0 && (my_struct->minus2 = 1))
				my_struct->star2 *= (-1);
			my_struct->len += 1;
			*i += 1;
		}
