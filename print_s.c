...
/*   Updated: 2020/02/20 01:36:45 by tembu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				print_s_space_precision(const char *str, int pos_after_percent, char **to_print)
{
	...
	i = 0;
	nb_precision = 0;
	nb_precision = small_atoi(str, &pos_save, nb_precision);
...
int				print_s(const char *str, t_flag my_struct, va_list args, int pos_after_percent)
{
	...
	//PRINTF("%3.s", NULL)
	if ((my_struct.zero == 0 && my_struct.minus == 0 && my_struct.nb > 0 && my_struct.nb2 == 0 && my_struct.precision == 1)
			|| (my_struct.zero == 0 && my_struct.minus == 1 && my_struct.nb > 0 && my_struct.nb2 == 0 && my_struct.precision == 1))
	...
	// PRINTF("%.2s", "salut")
			...
			&& my_struct.nb2 > 0 && my_struct.precision == 1)
		len += print_s_precision(str, pos_after_percent, &to_print);
	
	//PRINTF("%-10s", "salut");
	else if ( my_struct.zero == 0 && my_struct.minus == 1 && my_struct.nb > 0
			&& my_struct.precision == 0)
